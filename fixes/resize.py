...
MoviePy Resize Fix
==================
This file contains a modification to the original MoviePy's resize effect.

Problem:
--------
An AttributeError occurs with the original MoviePy resize.py script when
it references 'Image.ANTIALIAS', which is no longer available in the PIL/Pillow library.

Solution:
---------
The 'Image.ANTIALIAS' filter has been replaced with 'Image.LANCZOS' which
serves a similar purpose and provides high-quality downsampling filtering.

Usage:
------
This file should replace the existing resize.py in your MoviePy installation,
typically located in 'venv\Lib\site-packages\moviepy\video\fx'. Ensure you have
a backup of the original file before making this change.

Replace Instructions:
---------------------
1. Backup the original resize.py located at venv\Lib\site-packages\moviepy\video\fx.
2. Replace the original resize.py with this modified version.
3. If MoviePy is updated in the future, this change might need to be reapplied.

Disclaimer:
-----------
Please note that modifying library files can cause unexpected behavior and is generally
not recommended. Use this fix at your own risk and always test thoroughly after applying the fix.

Original Issue:
---------------
For reference, the issue addressed by this modification may be tracked and discussed in
the official MoviePy GitHub repository's issues section. It's advisable to check there for
permanent fixes in future releases of MoviePy.
